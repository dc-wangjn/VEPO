...
class FQE(object):
    def __init__(self, seed,policy, stochastic = True, depth = 1, # policy to be evaluated
...
#         with self.mirrored_strategy.scope():
    ...
    def train(self, trajs, idx = 0, train_freq = 100, test_freq = 10, verbose=0, nn_verbose = 0, validation_freq = 10
        ...
        """
        states, actions, rewards, next_states,dones = [np.array([item[i] for traj in trajs for item in traj]) for i in range(5)]
...
#                 print('now the {}-iter'.format(iteration))
                ...
                if self.stochastic:
                    ...
                    targets = rewards + (1-dones)*self.gamma * np.sum(q_next_states * A_probs, 1)
                else:
                    _actions = self.policy.get_A(next_states)
                    targets = rewards + (1-dones)*self.gamma * q_next_states[range(len(_actions)), _actions]  #*(1.0-dones)
