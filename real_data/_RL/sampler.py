...
class InitialStateSampler():
    ...
    def sample(self, batch_size = 1):
        np.random.seed(self.seed)      #modified by wjn 5-22
#         self.seed += 1
...
class SimpleReplayBuffer():
    ...
    def __init__(self, trajs):
        ...
        self.s_max = np.max(self.states, axis = 0)
        self.s_min = np.min(self.states, axis = 0)
        self.s_norm = (self.states - self.s_min)/(self.s_max - self.s_min)
        self.sn_norm = (self.next_states - self.s_min)/(self.s_max - self.s_min)
    ...
    def sample_norm(self, batch_size):
        ...
        if self.N > batch_size:
            ...
            idx = np.random.choice(self.N, batch_size, replace = True)
        return [self.s_norm[idx], self.actions[idx], self.rewards[idx], self.sn_norm[idx]]