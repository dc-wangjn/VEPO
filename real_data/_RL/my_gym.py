...
class GymEval():
    ...
    def simu_trajs(self, pi, rep = 100, T = 1000
        ...
        for i in range(rep):
            if t * 2 % rep == 0:
                print("simu {}% DONE!".format(str(i / rep * 100)))
            for t in range(T):
                ...
                SARS = [Ss[i].copy(), As[i], reward, SS.copy(),done]
                Ss[i] = SS
                if not done:
                    trajs[i].append(SARS)
    ...
    def simu_trajs_para(self, pi, rep = 100, T = 1000
        ...
        for t in range(T):
            ...
            if self.random_pi:
                ...
                As = pi.get_A(Ss)
            SSs, Rs, dones, _ = env.step_multiple(As)
            done = done+dones.astype(np.int)
            for i in range(rep):
                SARS = [Ss[i].copy(), As[i], Rs[i], SSs.T[i].copy(),dones[i].copy()]
                if done[i] <=1:
                    trajs[i].append(SARS)
