...
def eval_Ohio_pi(Q_func, J_Q, T, N, J_upper, sd_G = 0, gamma = 0.9, debug = 0, seed = 0):
    """ Evaluate the value of a policy in simulation.
    ...
    for t in range(J_upper, T): 
        ...
        # choose actions based on status. obs = [3, T, N]      
        s = ObsAct2State(obs, actions, t, J_Q, multiple_N = True) # dim * N
        prob = pi(s.T) # s [N * dx] -> pi [N * n_actions] -> 1 * N
        a_r = prob.shape[1]
        A_t = np.array([np.random.choice(a_r, 1, p = p_action)[0] for p_action in prob])
        actions[t, :] = A_t
...
#     r = []
#     for i in range(N):
#         rewards = np.roll(apply_v(Glucose2Reward, obs[0, J_upper:, i]), shift = -1).reshape(-1, 1)
#         r.append(rewards)
