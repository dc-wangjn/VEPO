...
#         noise_v = tf.cast(self.v_dist.sample(test_z.shape[0], self.seed), tf.float64)
        noise_v = tf.cast(np.zeros((test_z.shape[0],3)),tf.float64)# modified by wjn 5-6
...
def learn_cond_sampler(trajs=None, num_A= None, epochs=100, non_overlap_dim=3
    ...
    generator_x = linear_gaus_transition(z_dim, h_dims, v_dims, x_dims, batch_size,cov,use_aitken )
