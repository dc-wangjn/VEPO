...
class VisitationRatioModel_init_SASA():
    ...
    def __init__(self, seed,eps,kernel, replay_buffer
...
#         self.kernel = kernel
        if self.kernel:
            self.rbf_feature = RBFSampler(gamma=1, n_components=h_dims,random_state = 10)
            self.model = KernelRBF(self.A_dims)
        else:
...
#                 return dist
    ...
    def _compute_loss(self, S1, A1, S2, A2, S11, A11, SS11, S22, A22, SS22
        ...
        with tf.device('/gpu:' + str(self.gpu_number)):
            ...
            ###### omega (normalization) ######
            if self.kernel:
                omega_11_1_r_t = self.model.call(self.rbf_feature.fit_transform(X11r_X1t.numpy())) # n2
                omega_22_2_r_t = self.model.call(self.rbf_feature.fit_transform(X22r_X2t.numpy()))
            else:
            ...
            with tf.GradientTape() as tape:
                ...
                for j in range(rep_loss):
                    with tf.device('/gpu:' + str(self.gpu_number)):
                        ...
                        ### mean
                        nmean = min(batch_size * 10, self.replay_buffer.N)
                        transitions = self.replay_buffer.sample(nmean)
                        ...
                        if self.kernel:
                            Xr_Xt = self.rbf_feature.fit_transform(Xr_Xt.numpy()) 
                            Xr_Xt2 = self.rbf_feature.fit_transform(Xr_Xt2.numpy())
#                             Xr_Xt = self.rbf_feature.fit_transform(Xr_Xt)
#                             Xr_Xt2 = self.rbf_feature.fit_transform(Xr_Xt2)
                        omega_r_t = self.model.call(Xr_Xt) # n2
                        omega_r_t2 = self.model.call(Xr_Xt2) # n2
#                         print(omega_r_t.shape)
#                         print(nmean)
#                         print(batch_size)
                        mean_omega = tf.reduce_mean(tf.reshape(omega_r_t, [nmean, batch_size]), 0)
                        mean_omega2 = tf.reduce_mean(tf.reshape(omega_r_t2, [nmean, batch_size]), 0)
...
##########################################################################
            if i % 5 == 0 and i >= 10:
                ...
                if i % print_freq == 0:
                    print("omega_SASA training {}/{} DONE! loss = {:.5f}".format(i, max_iter, mean_loss))
                if np.abs(mean_loss / opt_loss - 1) < 0.01:
                    cnt_tolerance += 1
                else:
                    cnt_tolerance =0
                if mean_loss < opt_loss:
...
#                 if mean_loss < 0 or mean_loss < self.losses[0] / 10:
                if mean_loss < 0 :
                    print('criterion 1')
                    print(mean_loss < 0)
                    print(mean_loss < self.losses[0] / 10)
                    break
            elif i < 10 and i % 5 == 0 and print_freq < 200:
                print("omega_SASA training {}/{} DONE! loss = {:.5f}".format(i, max_iter, loss.numpy()))
                # print("loss for iter {} = {:.2f}".format(i, loss.numpy()))
#             print('cnt_tolerance is {}'.format(cnt_tolerance))
            if cnt_tolerance >= tolerance:
                print('criterion 2')
##########################################################################
##########################################################################
    ...
    def predict(self, inputs, to_numpy = True, small_batch = True, batch_size = int(1024 * 8 * 16)):
        with tf.device('/gpu:' + str(self.gpu_number)):
            if inputs.shape[0] > batch_size:
                ...
                input_batches = [self.rbf_feature.fit_transform(s) for s in input_batches]
                return np.vstack([self.model.call(inputs).cpu().numpy() for inputs in input_batches])
            else:
                inputs = self.rbf_feature.fit_transform(inputs)
