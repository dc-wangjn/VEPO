...
class RegularActor(nn.Module):
    ...
    def forward(self, state, prob_only = False):
        ...
        if prob_only:
            ...
            sample_As = torch.multinomial(A_prob,1,replacement=True)

            return sample_As.to(torch.float32) 
...
#         sampled_thre = torch.FloatTensor(np.random.rand(A_prob.size(0), num_sample))
        ...
        sample_As = torch.multinomial(A_prob,num_sample,replacement=True)
#         sample_As = torch.stack([((A_prob.cumsum(1) > sampled_thre[:,i].unsqueeze_(-1).to(self.device)).double().cumsum(1)==1).double().argmax(1) for i in range(num_sample)],1)
...
class BEAR(object):
    def __init__(self, num_qs = 2, state_dim = 3, action_dim = 1, latent_dim = 32, num_A = 5, delta_conf=0.1, use_bootstrap=True, version=0, lambda_=0.4,
        ...
        self.actor = RegularActor(state_dim, action_dim, latent_dim, num_A, 2).to(self.device)
        self.actor_target = RegularActor(state_dim, action_dim, latent_dim, num_A, 2).to(self.device)
