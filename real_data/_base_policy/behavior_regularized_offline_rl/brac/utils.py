...
def traj2dataset(trajs,gamma,batch_size):
  ob_dim = trajs[0][0][0].shape[0]
  flatten_traj = sum(trajs,[])
  data_size = len(flatten_traj)
...
#   data_ckpt = tf.train.Checkpoint(data=data)
  ...
  for traj in trajs:
    x = np.array(traj)
    y = np.hstack([x[:-1],x[1:,1][:,None]])
    s = np.concatenate([t[0].reshape(1,-1) for t in y],axis=0).astype(np.float32)
    s_ = np.concatenate([t[3].reshape(1,-1) for t in y],axis=0).astype(np.float32)
    a = np.concatenate([t[1].reshape(1,-1) for t in y],axis=0).astype(np.float32)
    a_ = np.concatenate([t[4].reshape(1,-1) for t in y],axis=0).astype(np.float32)
    r = np.concatenate([t[2].reshape(-1) for t in y],axis=0).astype(np.float32)

    traj_len = len(y)
    for k in range(int(traj_len/batch_size)):
        st_idx = k * batch_size
        batch = y[st_idx:st_idx+batch_size]
        disc = np.logspace(st_idx+1,st_idx+batch_size,batch_size,base=gamma).reshape(-1).astype(np.float32)
        transition = Transition(s[st_idx:st_idx+batch_size],
                                s_[st_idx:st_idx+batch_size],
                                a[st_idx:st_idx+batch_size],
                                a_[st_idx:st_idx+batch_size]
                                ,disc,
                                r[st_idx:st_idx+batch_size])
        data.add_transitions(transition)
