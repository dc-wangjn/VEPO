...
import sys
sys.path.append("/home/jianing/value_enhancement/_base_policy/")
...
class Agent(agent.Agent):
    ...
    def _build_a_loss(self, batch):
        ...
        div_estimate = self._divergence.dual_estimate(
            s, a_p, a_b, self._c_fn)
        a_loss = - tf.reduce_mean(alpha *
                                  (div_estimate - self._target_divergence))
    ...
    def train(self,replay_buffer,iterations,batch_size=100):
        for it in range(iterations):
            batch = replay_buffer.my_sample(batch_size)
...
class AgentModule(agent.AgentModule):
    """Tensorflow module for BRAC dual agent. Used by the class above """
