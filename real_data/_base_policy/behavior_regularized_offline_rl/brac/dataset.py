...
class my_Dataset(tf.Module):
  ...
  def __init__(
    ...
    self._size = size
    self._circular = circular
    self._s1 = self._zeros([size] + [obs_dim], tf.float32)
    self._s2 = self._zeros([size] + [obs_dim], tf.float32)
    self._a1 = self._zeros([size] + [action_dim], tf.float32)
    self._a2 = self._zeros([size] + [action_dim], tf.float32)
    self._discount = self._zeros([size], tf.float32)
    self._reward = self._zeros([size], tf.float32)
    self._data = Transition(
        s1=self._s1, s2=self._s2, a1=self._a1, a2=self._a2,
