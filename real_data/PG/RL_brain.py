...
class PolicyGradient:
    ...
    def _build_net(self):
        ...
        self.all_act_prob = tf.nn.softmax(self.all_act, name='act_prob') + 1e-5  # use softmax to convert to probability
    ...
    def choose_action(self, observation):
        ...
        p[np.isnan(p)] = 0
        p /= np.sum(p)
        action = np.random.choice(range(len(p)), p = p)  # select action w.r.t the actions prob
        return action
    ...
    def learn(self):
        ...
        # train on episode
        self.sess.run([self.train_op, self.check], feed_dict={
             self.tf_obs: np.vstack(self.ep_obs),  # shape=[None, n_obs]
             self.tf_acts: np.array(self.ep_as),  # shape=[None, ]
             self.tf_vt: discounted_ep_rs_norm,  # shape=[None, ]
        })
        #. tf.Print(self.loss, [self.loss], message="my Z-values:")
