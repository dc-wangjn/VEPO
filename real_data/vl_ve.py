...
# import simulation.vl_simulator as vl_simulator
import simulation.vl_simulator_mod as vl_simulator
...
from _base_policy.BCQ import bcq
...
from _density import GAN, omega_SASA_new, gaussian_vl #old version
...
class ValueEnhancement():
    ...
    def __init__(self,sigma, normalize, behav=None,non_overlap_dim=2,simulator='ohio',
...
#         if method == "BEAR":
        ...
        generator_rand = vl_simulator.simple_generator(gamma=self.gamma,sigma = self.vlsigma)
        trajs_rand = generator_rand.sample_behav_traj(self.seed,N_o,T_o,50)
###################################################################################
        ...
        if self.normalize :
            ...
            self.s_max = np.ones(states.shape[1])
            self.s_min = np.zeros(states.shape[1])
        ...
        if method == 'BEAR':
            ...
            optimal_pi_value_func = FQI_module.FQI(seed=self.seed,num_actions=self.num_A, gamma=self.gamma,init_states=self.init_state , **kwargs)
            opt_time = now()
    ...
    def init_normalization(self, trajs):
        ...
        return trajs_norm
    def apply_noise(self, trajs,sigma):
        ...
        rewards = rewards + sigma * np.random.randn(rewards.shape[0])
        trajs_norm = [[] for i in range(N)]
        for t in range(T):
            for i in range(N):
                trajs_norm[i].append([states[t+i*T],actions[t+i*T],
                                      rewards[t+i*T],next_states[t+i*T]])
        return trajs_norm    
    def generate_behave(self,N,T,sigma):
        
        
        ## some useful properties for massing data
        generator_rand = vl_simulator.simple_generator(gamma = self.gamma,sigma = self.vlsigma)
        trajs_rand = generator_rand.sample_behav_traj(self.seed,10000,100,50)
    
        self.S_dims = len(trajs_rand[0][0][0])
        init_state = np.array([x[0][0] for x in trajs_rand])
        states, actions, rewards,next_states = [
            np.array([item[i] for traj in trajs_rand for item in traj]) for i in range(4)]
        ...
        ## the generator used in this setting 
        self.N, self.T = N,T 
#         self.epsilon = epsilon
        self.generator = vl_simulator.simple_generator(gamma= self.gamma,s_min = self.s_min,s_max=self.s_max,sigma=self.vlsigma)

  
        trajs = self.generator.sample_behav_traj(self.seed,N,T,50)
        trajs = self.apply_noise(trajs, sigma)
        ...
        states, self.actions, self.rewards, self.next_states = [
...
#         self.split_ind = sample_split(self.L, self.N)  # split_ind[k] = {"train_ind" : i, "test_ind" : j}
        ...
        print('one sample state is {}, while s_min is {}, s_max is {}'.format(self.trajs[0][0][0],self.s_min, self.s_max))

    ################################################################################################



    def learn_init_policy(self, kernel=True, method=None, min_q_weight=1.0,train_iter=1e2, do_CV=False, latent_dim=32,
        ...
        if method == "BEAR":
            ...
            curr_policy = bear_old.BEAR(seed = self.seed,num_qs=2,latent_dim=latent_dim,
                                            ...
                                            lambda_=0.0, threshold=0.05, 
                                            mode="auto", num_samples_match=5, 
                                            mmd_sigma=10.0,lagrange_thresh=10.0, 
                                            use_kl=False, use_ensemble=True, 
                                            kernel_type='laplacian')
            print('replay_buffer is {}'.format(replay_buffer.sample(10)))
            curr_policy.train(replay_buffer, iterations=int(train_iter))  # pol_vals =
        elif method == "CQL":

            curr_policy = CQL_module.CQL(kernel=kernel, min_q_weight=min_q_weight,seed=self.seed,num_actions=self.num_A, gamma=self.gamma,init_states=self.init_state , **kwargs)
            opt_time = now()
...
#                 curr_policy.train(self.trajs, verbose=verbose, test_freq=test_freq,train_freq=train_freq, nn_verbose=nn_verbose, validation_freq=validation_freq)
            ...
            curr_policy = FQI_module.FQI(seed=self.seed,kernel=kernel, num_actions=self.num_A, gamma=self.gamma,init_states=self.init_state , **kwargs)
            opt_time = now()
            print('Start train init policy...')
            curr_policy.train(self.trajs, verbose=verbose, test_freq=test_freq,train_freq=train_freq, nn_verbose=nn_verbose, validation_freq=validation_freq)
#                 curr_policy.train(self.trajs, verbose=verbose, test_freq=test_freq,train_freq=train_freq, nn_verbose=nn_verbose, validation_freq=validation_freq)
        ...
        #### in order to keep the algorithm running, we have to use an alternative one.(CQL)
            ...
            try:
                ...
                vlearn = vl_simu.train_vl(trajs = self.trajs, gamma = self.gamma,num_A=self.num_A,seed = self.seed)
#                     vlearn = vl_simu.train_vl(trajs = self.trajs, gamma = self.gamma,num_A=self.num_A,seed = self.seed)

                print('Start train vl policy...')
                curr_time = now()
                curr_policy = vlearn.train()
                print('beta is {}'.format(curr_policy.beta))
            except:
...
#                     curr_policy.train(self.trajs, verbose=verbose, test_freq=test_freq,train_freq=train_freq, nn_verbose=nn_verbose, validation_freq=validation_freq)
                ...
                print('train done!')                   
            printG("<------  vlearning DONE! Time cost = {:.1f} minutes -------->\n".format( (now() - curr_time) / 60))

        elif method == "behav":
            curr_policy = self.behav
        elif method == "last":
    ...
    def OPE(self, kernel=True, verbose=1, eval_N=1000
...
#             else:
            ...
            curr_pi_value_func = FQE_module.FQE(seed=self.seed, kernel=kernel, policy=self.curr_policy[k],  num_actions=self.num_A, gamma=self.gamma,init_states=self.init_state , **kwargs)
            curr_pi_value_func.train([self.trajs[i] for i in train_ind], verbose=verbose, test_freq=test_freq,train_freq=train_freq, nn_verbose=nn_verbose, validation_freq=validation_freq)
    ...
    def est_density_ratio(self, kernel=True,h_dims=32, max_iter=100, batch_size=32,eps=0.01,print_freq=5):  # `omega_SA.py`
        ...
        for k in range(self.L):
            ...
            """
            omega_func = omega_SASA_new.VisitationRatioModel_init_SASA(seed=self.seed,kernel=kernel,
    ...
    def est_transition_density(self, v_dims=int(3), h_dims=int(32), epochs=100, batch_size=64, lr=0.0005, quantile=95,
        ...
        if trans_type == 'gauss':
            ...
            for k in range(self.L):
                curr = now()
                train_ind = self.split_ind[k]["train_ind"]
                gan = gaussian_vl.learn_cond_sampler(trajs=[self.trajs[i] for i in train_ind],num_A = self.num_A, epochs=epochs,
    ...
    def get_lower_bound_components(self, NN=10, TT=20, M=10, G_ds_n_seeds=1e3, use_ori_trans=False):
        ...
        for k, split, G, old_value, omega in zip(range(self.L), self.split_ind.values(), self.GANs
            ...
            if use_ori_trans:
                ...
                gene_S = self.generator.sample_S_4VE(pi=self.curr_policy[k],NN=NN, TT=TT, seed = self.seed,init_sampler = self.init_sampler)
                gene_S = (gene_S - self.s_min)/(self.s_max - self.s_min)
                
                gene_cond_S, gene_cond_S_star = self.generator.sample_cond_S_4VE(states, actions, next_states,self.curr_policy[k], M, TT ,seed = self.seed)
...
#         else:
        ...
        self.policy = policy.Policy(lamb = lamb, S_dims=self.S_dims, h_dims=h_dims, A_dims=self.A_dim, num_A=self.num_A,init_paras=init_paras, depth=depth)
...
#                 print(pi)
                value = self.generator.eval_policy(pi=pi,init_sampler = self.init_sampler,seed =42)    #modified by wjn 5-23
                print('{} policy value is {}'.format(policy,value))
                ...
                for k in range(self.L):
                    ...
                    value = self.generator.eval_policy(pi=pi,init_sampler = self.init_sampler,seed = 42)    #modified by wjn 5-23
                    values.append(value)
                print('{} policy value is {}'.format(policy,np.mean(values)))
                value = np.mean(values)
        
            else:
                # for behav, no need to evaluate the learned behav, can directly use the DGP to generate a lot!
                eval_trajs = self.generator.sample_behav_traj(42,20000,100,50)  # [N,T,SASA]
                value = self.generator.eval_trajs(eval_trajs)
                print('true behave policy value is {}'.format(value))
        return value    
