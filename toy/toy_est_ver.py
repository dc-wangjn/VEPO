...
#         print('sample_trajs using trans {}'.format(trans))
    ...
    def cal_q_function(self, pm, num_trajs, tt):
        if pm is None:
            s = np.array([0, 1])
            pm = self.new_pi.get_A_prob(s)
        
        trajs = self.sample_trajectory(num_trajs,tt, self.pm)
        state = trajs[:,:-1,0].reshape(-1).astype(np.int)
        action = trajs[:,:-1,1].reshape(-1).astype(np.int)
        next_state = trajs[:,1:,0].reshape(-1).astype(np.int)
        reward = self.reward(trajs[:,:-1,0],trajs[:,:-1,1],True).reshape(-1)

        ss = np.eye(self.state_dim)[state]
        aa = np.eye(self.action_dim)[action]
        ss_ = np.eye(self.state_dim)[next_state]
        ...
        sigma =  np.mean(np.stack([np.dot(x[:,None],y[None,:]) for x,y in zip(xi,res)]),axis=0) + 0.000001 * np.eye(self.state_dim*self.action_dim)
        # print(sigma)
        ...
        q = np.squeeze(beta).reshape(self.action_dim,self.state_dim,order='F') # (num_A, num_S)

        return q

    def cal_A_function(self,pm, num_trajs, tt):

        if pm is None:
            s = np.array([0, 1])
            pm = self.new_pi.get_A_prob(s)
        q = self.cal_q_function(pm, num_trajs, tt)
        v = np.sum(q * pm, axis = 1).reshape(-1,1)
        return q - v , v.reshape(-1)

    def cal_density(self, num_trajs, tt, pm_old):
        ...
        trajs = self.sample_trajectory(num_trajs,tt, self.pm)
        s = trajs[:,:-1,0].reshape(-1).astype(np.int)
        a = trajs[:,:-1,1].reshape(-1).astype(np.int)
        s_ = trajs[:,1:,0].reshape(-1).astype(np.int)
        s0 = trajs[:,0,0].reshape(-1).astype(np.int)

        ss = np.eye(self.state_dim)[s]
        aa = np.eye(self.action_dim)[a]
        phi = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(ss,aa)])

        pi_ = pm_old[s_]
        ss_ = np.eye(self.state_dim)[s_]
        pi_ss_ = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(ss_,pi_)])


        res = phi - self.gamma*pi_ss_
        sigma =  np.mean(np.stack([np.dot(x[:,None],y[None,:]) for x,y in zip(res,phi)]),axis=0) + 0.000001 * np.eye(self.state_dim*self.action_dim)
        sigma_inv = np.linalg.inv(sigma)

        pi0 = pm_old[s0]
        ss0 = np.eye(self.state_dim)[s0]
        _phi = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(ss0,pi0)])
        tgt = np.mean(_phi,axis=0)*(1-self.gamma)

        beta = np.dot(sigma_inv,tgt[:,None])
        
        w = np.squeeze(beta).reshape(self.state_dim,self.action_dim,order= 'F') # 
        m = np.mean(w[s,a])
        w = w/m
        return w

    def cal_trajs_density(self, trajs, pm_old):
        ...
        w = w/m
        return w.reshape(-1)
    def cal_dr(self,num_trajs, tt, pm_old):

        trajs = self.sample_trajectory(num_trajs,tt, self.pm)
        w_cond = np.zeros((self.state_dim*self.action_dim , self.state_dim*self.action_dim))
        for s in range(self.state_dim):
            for a in range(self.action_dim):
                idx = np.where((trajs[:,0,0]==s)*(trajs[:,0,1]==a))[0]
                traj_cond = trajs[idx]
                w_cond[:,s*self.action_dim+a] = self.cal_trajs_density(traj_cond,pm_old)
        w = self.get_omega(w_cond,pm_old,self.init_dist)
        return w, w_cond

                

    def cal_density_ratio(self, num_trajs,tt, pm_old):
        if pm_old is None:
            s = np.array([0, 1])
            pm_old = self.new_pi.get_A_prob(s)
        ...
        ss = np.eye(self.state_dim)[s]
        aa = np.eye(self.action_dim)[a]
        ssaa = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(ss,aa)])
        _ss = np.eye(self.state_dim)[_s]
        _aa = np.eye(self.action_dim)[_a]
        _ssaa = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(_ss,_aa)])
        phi = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(ssaa,_ssaa)])

        pi_ = pm_old[s_]
        ss_ = np.eye(self.state_dim)[s_]
        pi_ss_ = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(ss_,pi_)])
        u_ = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(pi_ss_,_ssaa)])

        res = phi - self.gamma*u_
        sigma =  np.mean(np.stack([np.dot(x[:,None],y[None,:]) for x,y in zip(res,phi)]),axis=0) + 0.000001 * np.eye(self.state_dim*self.action_dim*self.state_dim*self.action_dim)
        sigma_inv = np.linalg.inv(sigma)


        pi0 = pm_old[s0]
        ss0 = np.eye(self.state_dim)[s0]
        sspi0 = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(ss0,pi0)])
        _phi = np.concatenate([np.kron(y,x).reshape(1,-1) for x,y in zip(sspi0,sspi0)])
        tgt = np.mean(_phi,axis=0)*(1-self.gamma)

        beta = np.dot(sigma_inv,tgt[:,None])
        
        w_cond = np.squeeze(beta).reshape(self.state_dim*self.action_dim,self.state_dim*self.action_dim,order= 'F') # 
        idx = s*self.action_dim+a
        m = np.mean(w_cond[idx],axis=0).reshape(1,-1)
        w_cond = w_cond/m
        w = self.get_omega(w_cond,pm_old,self.init_dist)
    ...
    def cal_transition(self, num_trajs, tt ):
        ...
        trans = np.zeros((self.state_dim*self.action_dim, self.state_dim))

        for i in range(len(trans)):
            s = i // self.action_dim
            a = i - s*self.action_dim
            idx = np.where((state==s)*(action==a))[0]
            count = len(idx)
            for j in range(self.state_dim):
                c = len(np.where(next_state[idx]==j)[0])
                trans[i,j] = c/count
