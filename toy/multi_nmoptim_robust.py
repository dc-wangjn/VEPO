...
#         s = np.array([0, 1])
        ...
        start_T = time.time()
        up_q = toy.cal_q_function(up_pm_old,cal_nu_nn,cal_nu_mm,cal_nu_tt)
        ...
        trans = None
        noise1 = np.random.random_sample(up_omega_cond.shape) * noise_var
        noise2 = np.random.random_sample(up_q.shape) * noise_var
        noise3 = np.random.exponential(1,TRUE_TRANS[:,0].shape)
        
        if mod == 'mod1':
#             noise = np.random.random_sample(up_omega_cond.shape) * noise_var
#             noise = np.random.exponential(1,up_omega_cond.shape)
            up_omega_cond = up_omega_cond + noise1
            up_omega = toy.get_omega(up_omega_cond,up_pm_old,toy.init_dist)
        if mod == 'mod2':
#             noise = np.random.exponential(1,up_q.shape)
#             up_q = up_q * noise
#             up_a_func,up_v_func = toy.get_A(up_q,up_pm_old)
#             noise = np.random.random_sample(up_q.shape) * noise_var
            up_q = up_q + noise2
            up_a_func,up_v_func = toy.get_A(up_q,up_pm_old)            
            
        if mod == 'mod3':
            trans = copy.deepcopy(TRUE_TRANS)
#             noise = np.random.exponential(1,trans[:,0].shape)
            trans[:,0] = np.clip(trans[:,0]*noise3,0,0.99)
            trans[:,1] = 1- trans[:,0] 
        if mod == 'mod4':
#             noise = np.random.random_sample(up_omega_cond.shape) * noise_var
            up_omega_cond = up_omega_cond + noise1
...
#             noise = np.random.exponential(1,up_omega_cond.shape)
#             up_omega_cond = up_omega_cond * noise
            up_omega = toy.get_omega(up_omega_cond,up_pm_old,toy.init_dist)
...
def main(args):
    ...
    output_pth = 'results/multi_robust_sigma{}_delta{}_{}.pkl'.format(r_sigma,args.delta,args.suffix)
#     NN_TT = [[10,10],[50,50]]
#     NN_TT = ((10,10),(30,30))
    NN_TT = ((30,30),(50,50))
#     NN_TT = ((50,50),(70,70))
...
#     INIT_LOGLAMBDA2 = 5.7 #298
    ...
    print(config)
    result = np.zeros((len(GAMMA),K_iters+1,len(P_old),len(model),len(NN_TT),rep,2))
    lift = np.zeros((len(GAMMA),K_iters+1,len(P_old),len(model),len(NN_TT),rep,2))
    cr = np.zeros((len(GAMMA),K_iters+1,len(P_old),len(model),len(NN_TT),rep))
...
#             if pm_old.any() == p3.any():
            ...
            TRUE_TRANS = copy.deepcopy(toy.trans)
            start_T = time.time()
        #     a_func,v_func = toy.cal_A_function(pm_old,cal_nu_nn,cal_nu_mm,cal_nu_tt)
            ...
            #                 print(q)
            _, OMEGA_COND_TRUE = toy.cal_density_ratio(cal_nu_nn,cal_nu_mm,cal_nu_tt, pm_old ,toy.pm)
            ...
            for n3,mod in enumerate(model):
                omega,omega_cond = OMEGA_TRUE,OMEGA_COND_TRUE
                a_func,v_func = A_TRUE,V_TRUE
                trans = None
                
                
                noise1 = np.random.random_sample(omega_cond.shape) * noise_var
                noise2 = np.random.random_sample(q.shape) * noise_var
                noise3 = np.random.exponential(1,TRUE_TRANS[:,0].shape)
                if mod == 'mod1':
#                     noise = np.random.random_sample(omega_cond.shape) * noise_var
#                     noise = np.random.exponential(1,omega_cond.shape)
                    omega_cond = omega_cond + noise1
                    omega = toy.get_omega(omega_cond,pm_old,toy.init_dist)

                if mod == 'mod2':
#                     noise = np.random.exponential(1,q.shape)
#                     q = q * noise
#                     a_func,v_func = toy.get_A(q,pm_old)
#                     noise = np.random.random_sample(q.shape) * noise_var
                    q = q + noise2
                    a_func,v_func = toy.get_A(q,pm_old)
...
#                     noise = np.random.exponential(1,trans[:,0].shape)
                    ...
                    trajs_star = toy.sample_trajectory(M, T_star, pm_old,trans)

                    state = np.array([0,0,1,1])
                    action = np.array([0,1,0,1])
                    trajs_cond_temp = toy.sample_cond_trajectory(M2, T_star,pm_old,state,action,trans).reshape(4, M2, T_star, 2)
...
#                     print(res)
                    for i in range(rep):
                        for j in range(K_iters+1):
                            lift[n1,j,n2,n3,n5,i] = res[i][j][0]
                            result[n1,j,n2,n3,n5,i] = res[i][j][1]
                            cr[n1,j,n2,n3,n5,i] = res[i][j][2]
