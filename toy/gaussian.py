from real_data._density.GAN_utils import *
...
#         x_samples = np.random.multivariate_normal(x_mean.numpy(), x_var.numpy())
        ...
        if self.use_aitken == 'True':
            ...
            x_samples = np.dot(rditem,self.std) + x_mean.numpy()
        else:
            rditem = np.random.randn(x_mean.numpy().shape[0],x_mean.numpy().shape[1])
            x_samples = rditem + x_mean.numpy()
        
#         x_samples = np.random.randn(x_mean.shape) + x_mean
        self.seed += 1
        if self.non_overlap_dim > 0:
            return np.hstack([test_z[:, self.non_overlap_dim+1:], x_samples])
        else:
            return x_samples
...
def learn_cond_sampler(trajs=None, epochs=100, non_overlap_dim=3
    ...
    optimiser = tf.keras.optimizers.Adam(optimizer["lr"], beta_1=0.5, clipnorm=optimizer["gen_clipping_norm"],clipvalue=optimizer["gen_clipping_val"])
    ...
    generator_x = gaus_transition(z_dim, h_dims, v_dims, x_dims, batch_size,cov,use_aitken )

    @tf.function
    def x_update_g(real_x, real_z, v, gx_optimiser, generator_x):
    ...
    for x_batch, z_batch in batched_training.take(epochs):
        ...
        loss_x,var = x_update_g(x_batch, z_batch, noise_v, optimiser,generator_x)
...
#         print('iter {} generator loss ->{} '.format(current_iters, loss_x))
        ...
        current_iters += 1
    #####################################################################################################################
    cond_sampler = CondSampler(v_dist=v_dist, v_dims=v_dims, generator_x=generator_x, non_overlap_dim=non_overlap_dim)
    return cond_sampler
