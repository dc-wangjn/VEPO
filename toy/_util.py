...
def get_flat_params_from(param_list):    
    return tf.concat([tf.reshape(param,[-1]) for param in param_list],axis=0)


def set_flat_params_to(param_list, flat_params):
    prev_ind = 0
    for param in param_list:
        flat_size = int(np.prod(list(param.shape)))
        param.assign(
            tf.reshape(flat_params[prev_ind:prev_ind + flat_size],param.shape))
        prev_ind += flat_size